...
data = dict(
    ...
    val=dict(
        type='SARDataset',
        ann_file='data/ssdd/test/all/labelTxt/',
        img_prefix='data/ssdd/test/all/images/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
...
model = dict(
    ...
    bbox_head=dict(
        ...
        angle_coder=dict(
            type='PSCCoder', angle_version='le90', dual_freq=True,
            num_step=3)),
    train_cfg=None,
    test_cfg=dict(
        nms_pre=2000,
