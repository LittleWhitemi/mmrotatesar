...
class PSCRFCOSHead(RotatedFCOSHead):
    """Use `Phase-Shifting Coder (PSC)

    <https://arxiv.org/abs/2211.06368>`.`_ .
    in `FCOS <https://arxiv.org/abs/1904.01355>`_.

    Args:
        separate_angle (bool): If true, angle prediction is separated from
            bbox regression loss. In PSC only support True. Default: True.
            scale_angle (bool): If true, add scale to angle pred branch.
                In PSC only support False. Default: False.
    ...
    def __init__(self,
                 ...
                 angle_coder=dict(
                     type='PSCCoder',
                     angle_version='le90',
                     dual_freq=False, # 是否使用重频，默认False
                     num_step=3),   # 编码阶数 N >= 3
                 **kwargs):
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        labels, bbox_targets, angle_targets = \
            super(PSCRFCOSHead, self)._get_target_single(gt_bboxes,
                                                         gt_labels,
                                                         points,
                                                         regress_ranges,
                                                         num_points_per_lvl)
